...
public class ContactList<Person> implements ContactListADT<Person>
{
    ...
    private void addFirstName(Person personData)
    {
        head = new Node<>(personData, head);
        size++;
    }

    private void addLastName(Node<Person> personNode, Person personData)
    {
    ...
    public void add(int index, Person personData)
    {
        if (index < 0 || index > size)
        {
            ...
            addFirstName(personData);
        else
        {
            Node<Person> temp = getNode(index - 1);
            addLastName(temp, personData);
        }
    }
    ...
    private Person removeFirstName() {
    ...
    private Person removeLastName(Node<Person> node) {
        Person response = null;
